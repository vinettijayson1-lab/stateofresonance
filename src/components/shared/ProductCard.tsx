...
export default function ProductCard({ p, priority = false }: { p: ShopifyProduct, priority?: boolean }) {
  return (
    <div className="group w-full flex flex-col items-stretch relative">
      ...
      {/* Image + title link to PDP */}
      <Link href={`/product/${p.handle}`} className="flex flex-col items-stretch w-full">
      ...
      {/* Quick add-to-cart — sibling of link, click does NOT bubble to PDP */}
