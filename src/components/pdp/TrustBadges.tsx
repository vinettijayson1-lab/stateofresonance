    ...
    <div className="flex flex-col gap-0 w-full">
      ...
      <div className="grid grid-cols-2 gap-0">
        <button 
          ...
          className={`flex items-center justify-between p-3 border border-[rgba(255,255,255,0.05)] border-t-0 text-left text-[0.6rem] font-sans tracking-widest uppercase transition-colors ${activeBadge === 'shipping' ? 'bg-[rgba(212,175,55,0.05)] text-white' : 'bg-transparent text-[#7a7a7a] hover:bg-[rgba(255,255,255,0.02)]'}`}
        >
          <div className="flex items-center gap-2.5">
            <Truck className="w-3.5 h-3.5 text-[var(--color-gold-muted)] flex-shrink-0" />
            <span>Free Shipping 110+</span>
          </div>
          <ChevronDown className={`w-3 h-3 text-gray-500 transition-transform ${activeBadge === 'shipping' ? 'rotate-180' : ''}`} />
        </button>

        <button 
          onClick={() => toggleBadge('returns')}
          className={`flex items-center justify-between p-3 border border-[rgba(255,255,255,0.05)] border-t-0 border-l-0 text-left text-[0.6rem] font-sans tracking-widest uppercase transition-colors ${activeBadge === 'returns' ? 'bg-[rgba(212,175,55,0.05)] text-white' : 'bg-transparent text-[#7a7a7a] hover:bg-[rgba(255,255,255,0.02)]'}`}
        >
          <div className="flex items-center gap-2.5">
            <RotateCcw className="w-3.5 h-3.5 text-[var(--color-gold-muted)] flex-shrink-0" />
            <span>30-Day Returns</span>
          </div>
          <ChevronDown className={`w-3 h-3 text-gray-500 transition-transform ${activeBadge === 'returns' ? 'rotate-180' : ''}`} />
        </button>

        <button 
          onClick={() => toggleBadge('secure')}
          className={`flex items-center justify-between p-3 border border-[rgba(255,255,255,0.05)] border-t-0 text-left text-[0.6rem] font-sans tracking-widest uppercase transition-colors ${activeBadge === 'secure' ? 'bg-[rgba(212,175,55,0.05)] text-white' : 'bg-transparent text-[#7a7a7a] hover:bg-[rgba(255,255,255,0.02)]'}`}
        >
          <div className="flex items-center gap-2.5">
            <Lock className="w-3.5 h-3.5 text-[var(--color-gold-muted)] flex-shrink-0" />
            <span>Secure Checkout</span>
          </div>
          <ChevronDown className={`w-3 h-3 text-gray-500 transition-transform ${activeBadge === 'secure' ? 'rotate-180' : ''}`} />
        </button>

        <button 
          onClick={() => toggleBadge('canada')}
          className={`flex items-center justify-between p-3 border border-[rgba(255,255,255,0.05)] border-t-0 border-l-0 text-left text-[0.6rem] font-sans tracking-widest uppercase transition-colors ${activeBadge === 'canada' ? 'bg-[rgba(212,175,55,0.05)] text-white' : 'bg-transparent text-[#7a7a7a] hover:bg-[rgba(255,255,255,0.02)]'}`}
        >
          <div className="flex items-center gap-2.5">
            <Award className="w-3.5 h-3.5 text-[var(--color-gold-muted)] flex-shrink-0" />
            <span>Made in Canada</span>
          </div>
          <ChevronDown className={`w-3 h-3 text-gray-500 transition-transform ${activeBadge === 'canada' ? 'rotate-180' : ''}`} />
        </button>
      </div>

      {/* Active Badge Content */}
      {activeBadge && (
        <div className="p-4 border border-[rgba(255,255,255,0.05)] border-t-0 bg-[#0a0a0a]">
          <p className="text-[0.65rem] text-gray-300 font-sans tracking-wide leading-relaxed">
